...
【評価基準】
1. LLMの出力から最初の3つの薬剤候補のみを抽出し、個別に評価してください
2. 4つ以上の薬剤が含まれていても、最初の3つのみを評価対象としてください
3. 各薬剤について、薬剤名と説明（理由）が医学的に適切かどうかを評価してください
4. 評価対象は「薬剤の選択」と「薬剤の説明」のみです。文章のスタイルや追加情報は評価に含めないでください
...
- **overall.scoreは、3つの薬剤スコアの平均値を設定してください**
- overall.labelは以下の基準で設定:
  - "correct": overall.score >= 0.8
  - "partially_correct": 0.5 <= overall.score < 0.8
  - "incorrect": overall.score < 0.5
...
{{
    ...
    "overall": {{
        ...
        "is_harmful": false,
        "score": 0.93,  // 3つの薬剤スコアの平均値（例: (1.0 + 1.0 + 0.8) / 3 = 0.93）
